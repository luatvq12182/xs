            ...
            if (num1 === num2) {
                ...
                for (let i = 0; i < indexes[num1].length; i++) {
                    for (let j = 0; j < indexes[num2].length; j++) {
                        ...
                        if (
                            +`${prizes[index1[0]]}${index1[1]}${index1[2]}` <=
                            +`${prizes[index2[0]]}${index2[1]}${index2[2]}`
                        ) {
                            indexsNeedToCheck.push({
                                i1: index1,
                                i2: index2,
...
const lotoTheoThu = (req, res) => {
    try {
        const { province, numberOfDays, dayOfWeek } = req.query;

        let kqxs = CACHE.get("KQXS");

        if (province == Constants.Domain.MienBac) {
            kqxs = kqxs[Constants.Domain.MienBac];
        } else {
            kqxs =
                kqxs[2][province] ||
                kqxs[3][province === "Hồ Chí Minh" ? "TPHCM" : province];
        ...
        kqxs.reverse();
        kqxs = kqxs.slice(-(+numberOfDays + 1));
        kqxs.reverse();
        ...
        Array.from({ length: 100 }, (_, index) => {
            ...
            for (let i = 0; i < indexsNeedToCheck.length; i++) {
                ...
                for (let j = 1; j < rs.length; j++) {
                    ...
                    const numsReturn = rs[j - 1].nums.filter((e) => {
                        return (
                            e == numOfIndex ||
                            e == `${numOfIndex[1]}${numOfIndex[0]}`
                        );
                    });

                    if (numsReturn.length < 2) {
                        isValid = false;
                    }
                }
