...
const DOMAIN = "https://soicau888.io";
const USERNAME = "azngao888-io";
const PASSWORD = "ANF28Um85osP3x";

// const DOMAIN = "http://atrungroi.test";
// const USERNAME = "admin";
// const PASSWORD = "Bw0G&(H%fM8acIQ@CF";
...
const insertPostDomainLevel = async (page, domain, date, month) => {
    const a = {
        1: "XSMB",
        2: "XSMT",
        3: "XSMN",
    ...
    await page.type(
        "#title",
        `${a[domain]} ${date}/${month}/2024 – KQ Xổ Số Miền ${b[domain]} Ngày ${date}-${month}-2024`
    );
    await page.select("#acf-domain", "" + domain);

    await page.evaluate(
        async (domain, date, month, a, b) => {
            document.querySelector("#acf-date").value =
                "2024" +
    ...
    await page.evaluate(
        async (domain, date, month, a, b) => {
            ...
            document.querySelector("#new-post-slug").value = convertToSnakeCase(
                `KQ Xổ Số Miền ${b[domain]} Ngày ${date} ${month} 2024`
            );
...
const main = async () => {
    try {
        ...
        //     for (let date = 1; date <= MONTHS[month]; date++) {
        ...
        await insertPostDomainLevel(page, Constants.Domain.MienBac, DATE, MONTH);
        await insertPostDomainLevel(page, Constants.Domain.MienTrung, DATE, MONTH);
        await insertPostDomainLevel(page, Constants.Domain.MienNam, DATE, MONTH);
        await insertPostProvinceLevel(page, Constants.Domain.MienTrung, DATE, MONTH);
        await insertPostProvinceLevel(page, Constants.Domain.MienNam, DATE, MONTH);
...
const upBaiSoiCau888 = async (date, month, year) => {
    const browser = await puppeteer.launch({
        headless: false,
        defaultViewport: null,
        ignoreHTTPSErrors: true,
    ...
    if (errors.length > 0) {
        ...
        const filePath = "up-bai-kqxs-soicau888.error.txt";
